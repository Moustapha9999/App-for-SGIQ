...
class Achat(Base):
    ...
    lignes: Mapped[list["LigneAchat"]] = relationship(back_populates="achat", cascade="all, delete-orphan")
...
class Vente(Base):
    ...
    id_user: Mapped[int | None] = mapped_column(ForeignKey("utilisateurs.id_user"))
    numero_facture: Mapped[str | None] = mapped_column(String(40))

    client: Mapped["Client | None"] = relationship(back_populates="ventes")
    lignes: Mapped[list["LigneVente"]] = relationship(back_populates="vente", cascade="all, delete-orphan")

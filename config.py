...
# ── Base de données ──────────────────────────────────────────────────────
# Supabase en prod, SQLite en dev si DATABASE_URL absent
DEFAULT_DB = f"sqlite:///{DATA_DIR / 'sgiq.db'}"
DATABASE_URL = os.getenv("DATABASE_URL", DEFAULT_DB)

# Supabase renvoie parfois "postgres://" — SQLAlchemy exige "postgresql://"
if DATABASE_URL.startswith("postgres://"):
    DATABASE_URL = DATABASE_URL.replace("postgres://", "postgresql://", 1)

# ── App ──────────────────────────────────────────────────────────────────
TVA_DEFAULT    = float(os.getenv("TVA", "18"))
DEVISE_DEFAULT = os.getenv("DEVISE", "FCFA")
